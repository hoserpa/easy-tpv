  ...
  describe('/tickets (POST)', () => {
    it('should create a new ticket', () => {
      return request(app.getHttpServer())
        ...
        .then((response) => {
          ...
          expect(response.body.ticket.subtotal).toBe(25.00)
          expect(response.body.ticket.total).toBe(25.00)
          expect(response.body.lines).toHaveLength(2)
          expect(response.body.lines[0].total).toBe(20.00)
          expect(response.body.lines[1].total).toBe(5.00)
        })
    })
    ...
    it('should create ticket with line discounts', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect(201)
        .then((response) => {
          expect(response.body.ticket.total).toBe(15.00)
          expect(response.body.lines[0].total).toBe(15.00)
        })
    })
    ...
    it('should create ticket with ticket discount', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect(201)
        .then((response) => {
          expect(response.body.ticket.subtotal).toBe(100.00)
          expect(response.body.ticket.total).toBe(90.00)
        })
    })
  ...
  describe('/tickets/:id/lines (GET)', () => {
    ...
    it('should return ticket lines only', async () => {
      ...
      return request(app.getHttpServer())
        ...
        .then((response) => {
          expect(response.body).toHaveLength(2)
          expect(response.body[0].ticket_id).toBe(createResponse.body.ticket.id)
          expect(response.body[1].ticket_id).toBe(createResponse.body.ticket.id)
        })
    })
  })
})